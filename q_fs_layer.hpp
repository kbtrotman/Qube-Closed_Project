...
class qube_FS {
	...
	public:
        ...
        static char* qfs_get_root_path(const char* path) {
            ...
            qube_log::DEBUG("QUBE_FS::qfs_get_root_path: rtemp: {}, s: {}, fp: {}", rtemp, s, result);

            char *ftemp = new char[result.size() + 1];
            if ( *result.c_str() == '\0' ) {
                qube_log::DEBUG("QUBE_FS::qfs_get_root_path: returning path is NULL = {}", result.c_str());   
            } else {
                qube_log::DEBUG("QUBE_FS::qfs_get_root_path: resolving path = {}", result.c_str());
                ::realpath(result.c_str(), ftemp);
            }

            qube_log::DEBUG("QUBE_FS::qfs_get_root_path: rootdir = {}, rel path = {}, full path = {}", settings.root_dir->c_str(), path, ftemp);
            qube_log::TRACE("QUBE_FS::qfs_get_root_path:---[Leaving]--->");
            qube_log::FLUSH;
            return ftemp;
        }
		...
		static int qfs_write_to_file( int fd, const char *data_content, size_t size, off_t offset ) {
            ...
            if (write_result == -1) {
                qube_log::DEBUG("QUBE_FS::qfs_Write_to_file: error writing to filehandle---[{:d}]--->", fd);
            } else {
                qube_log::DEBUG("QUBE_FS::qfs_Write_to_file: wrote {} bytes to filehandle {:d} at offset {}, total now written = {} bytes.", size, fd, offset, write_result);
            }

            ::close(fd);
            qube_log::TRACE("QUBE_FS::qfs_write_to_file:---[Leaving]--->");
            qube_log::FLUSH;
            return write_result;
		}
