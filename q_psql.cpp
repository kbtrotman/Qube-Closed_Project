...
#include "q_psql.hpp"
    ...
    int q_psql::qpsql_incr_hash_count(std::string hash) {
        ...
        rec_count = 0;
        TRACE("QUBE_PSQL::qpsql_incr_hash_count---[Leaving]---with record count: {:d}--->", q_psql::rec_count);
        FLUSH;
        return rec_count;
    }
    ...
    PGresult* q_psql::qpsql_execQuery(std::string qube_query) {
        ...
        TRACE("QUBE_PSQL::qpsql_execQuery---[Leaving]---with record count: {:d}--->", q_psql::rec_count);
        FLUSH;
        return res;
    }
